...
proc decrypt(secret:SecretKey,encrypted:cstring,len:uint):string=
    var 
        cbc : CBC[aes128]
    ...
    cbc.init(secret.string.toOpenArrayByte(0, secret.string.len-1), cbcIV.toOpenArrayByte(0, cbcIV.len-1))
    cbc.decrypt(cast[ptr byte](encrypted), cast[ptr byte]( decrypted), len)
    cbc.clear()
    result = $decrypted
...
when isMainModule:
    let
        ...
        message: cstring="""hello Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do" #eiusmod tempor incididunt ut labore et dolore magna aliqua. 
